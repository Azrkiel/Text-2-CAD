...
STRATEGY_D = (
    ...
    "\n"
    "  # CORRECT — each body is a separate dict entry: "
    "  parts = {} "
    "  parts['wing_skin'] = skin "
    "  parts['rib_1'] = rib_1 "
    "  parts['rib_2'] = rib_2 "
    "  parts['spar_front'] = spar_front "
    "  parts['spar_rear'] = spar_rear "
    "  result = parts  # DOMAIN D OVERRIDE: result is a dict, not a single Workplane "
    "\n"
    "DOMAIN D OVERRIDE — result variable: "
    "Universal Rule (2) requires result to be a single cq.Workplane. For Domain D, "
    "this rule is OVERRIDDEN. Set result = parts (a dict mapping part name strings "
    "to cq.Workplane objects). The Assembler agent reads this dictionary and mates "
    "each body using topological anchors from the AssemblyManifest. "
    "\n"
    ...
    "\n"
    "(D1) RIB GENERATION: Ribs are thin plates perpendicular to the span axis. "
    "Each rib blank MUST be intersected with the PRE-SHELL solid (outer_wp), "
    "NOT the shelled skin. The pre-shell solid defines the full airfoil cross- "
    "section; intersecting with it gives each rib its correct internal profile. "
    "Each rib is stored as a separate entry in the parts dict. "
    "\n"
    "(D2) SPAR GENERATION: Spars run the full span at fixed chordwise positions "
    "(typically 25% and 60% chord from the leading edge). Same principle: "
    "intersect with the pre-shell solid (outer_wp). Store each spar as a "
    "separate entry in the parts dict. "
    "\n"
    "(D3) COMPLETE WORKED EXAMPLE — hollow skin + separate ribs + spars: "
    "  from cad_utils import make_naca_wire "
    ...
    "  skin = outer_wp.faces('<Y').shell(-wall_thickness) "
    "\n"
    "  # --- Step C: Generate ribs (intersect with PRE-SHELL solid) --- "
    ...
    "      ) "
    "      parts[f'rib_{i}'] = rib_blank.intersect(outer_wp) "
    "\n"
    "  # --- Step D: Generate spars (intersect with PRE-SHELL solid) --- "
    ...
    "      ) "
    "      parts[name] = spar_blank.intersect(outer_wp) "
    "\n"
    "  # --- result = parts dict (NO .union()) --- "
    "  result = parts "
    "\n\n"
