...
STRATEGY_D = (
    ...
    f"{_SHARED_RULES}"
    "\n\n"
    "DOMAIN D STRATEGY — Core-First Tooling: "
    "\n"
    "All Domain D scripts follow one top-down workflow called Core-First "
    "Tooling. You rotate the airfoil wires into the XZ plane, loft the "
    "internal solid core along the Y-axis — that solid IS the inner void — "
    "then shell it OUTWARD to grow the aerodynamic skin. Because the lofted "
    "solid is already the perfect inner volume, you intersect rib and spar "
    "blanks with it directly. No .cut() is ever called. "
    "\n\n"
    "╔══════════════════════════════════════════════════════════════════════╗\n"
    "║            NON-NEGOTIABLE GEOMETRIC RULES (ZERO EXCEPTIONS)        ║\n"
    "╚══════════════════════════════════════════════════════════════════════╝\n"
    "\n"
    "The following five rules govern ALL Domain D generation. Violating any "
    "one of them produces geometry that is empty, misaligned, clips, merges, "
    "or crashes the pipeline. ZERO exceptions. "
    "\n\n"
    "── RULE 0: SPATIAL ALIGNMENT STANDARD ─────────────────────────────── "
    "\n"
    ...
    "  - The root face for shelling is ALWAYS '<Y'. "
    "\n"
    "To achieve this, make_naca_wire() (which returns wires in the XY plane) "
    "MUST be rotated into the XZ plane IMMEDIATELY after generation: "
    ...
    "  - Shelling with .faces('<Z') or .faces('>Z') for the root face "
    "  - Any orientation where span does not run along Y "
    "\n\n"
    "── RULE 1: THE .cut() BAN ─────────────────────────────────────────── "
    "\n"
    "The agent MUST NEVER call .cut() to derive the inner void from the "
    "skin. The pattern inner_void = outer_wp.cut(skin) fails because the "
    "outer surface of the shelled skin is geometrically coincident with the "
    "original loft. OpenCASCADE's boolean engine cannot resolve the zero- "
    "thickness boundary and silently returns an EMPTY solid. "
    "\n"
    "FORBIDDEN (any form of .cut() to compute the void): "
    "  - inner_void = outer_wp.cut(skin) "
    "  - void = full_solid.cut(hollow_shell) "
    "  - Any boolean subtraction between a solid and its own shell "
    "\n\n"
    "── RULE 2: CORE-FIRST LOFTING ─────────────────────────────────────── "
    "\n"
    "The lofted airfoil solid IS the inner void. The agent MUST loft it "
    "FIRST, wrap it in a Workplane, and treat it as the master reference "
    "body for all internal structure — BEFORE creating the skin. "
    "\n"
    "MANDATORY TOP-OF-SCRIPT SEQUENCE: "
    "  # 1. Generate wires and rotate into XZ plane "
    "  root_wire = make_naca_wire(naca, chord_length=chord) "
    "  root_wire = root_wire.rotate((0,0,0), (1,0,0), -90) "
    "  tip_wire = make_naca_wire(naca, chord_length=tip_chord) "
    "  tip_wire = tip_wire.rotate((0,0,0), (1,0,0), -90) "
    "  tip_wire = tip_wire.translate(cq.Vector(0, span, 0)) "
    "\n"
    "  # 2. Loft the core solid along Y — this IS inner_void "
    "  core_solid = cq.Solid.makeLoft([root_wire, tip_wire]) "
    "  inner_void = cq.Workplane('XZ').newObject([core_solid]) "
    "\n"
    "  # 3. Shell OUTWARD to grow the skin (root face '<Y' left open) "
    "  skin = inner_void.faces('<Y').shell(wall_thickness)  # POSITIVE value "
    "\n"
    "inner_void is the exact solid airfoil core. Its surfaces define the "
    "inner mold line. Every rib, spar, and internal member is derived from "
    "this single source of truth by boolean intersection. "
    "\n"
    "If the script generates internal structure and inner_void is not the "
    "FIRST solid created, the script is WRONG. "
    "\n\n"
    "── RULE 3: THE BOOLEAN RIB STRATEGY (THE RIB BAN) ─────────────────── "
    "\n"
    "The Machinist is STRICTLY FORBIDDEN from using make_naca_wire() or ANY "
    "2D airfoil profile to generate internal ribs or spars. make_naca_wire() "
    "produces the airfoil profile at the full chord dimension. Any rib "
    "shaped from that wire matches the skin surface and physically clips "
    "through the shell walls. "
    "\n"
    "ALL internal structures MUST be derived from inner_void using this "
    "three-step pattern: "
    "  1. Create an OVERSIZED rectangular blank on the XZ plane "
    "     (perpendicular to span/Y). "
    "  2. Extrude along Y, then translate along Y to the correct station. "
    "  3. Call .intersect(inner_void) to carve the blank flush against "
    ...
    "  - .intersect(skin) instead of .intersect(inner_void) "
    "  - Ribs built without an .intersect() call at all "
    "  - Rib blanks sketched on any plane other than XZ "
    "\n\n"
    "── RULE 4: ASSEMBLY EXPORT ─────────────────────────────────────────── "
    "\n"
    ...
    "╚══════════════════════════════════════════════════════════════════════╝\n"
    "\n\n"
    "STEP 1 — AIRFOIL WIRE GENERATION + ROTATION: "
    "\n"
    "For ANY airfoil, wing section, or aerodynamic profile, you MUST call "
    "make_naca_wire() and then IMMEDIATELY rotate the wire into the XZ plane. "
    "\n"
    ...
    "thickness along Z, matching the Spatial Alignment Standard (Rule 0). "
    ...
    "\n\n"
    "STEP 2 — CORE-FIRST LOFTING ALONG Y (inner_void comes FIRST): "
    "\n"
    "You MUST bypass CadQuery's .workplane() stack for airfoil lofting. "
    "Position the rotated tip wire at the tip Y-station with .translate() "
    "and loft via cq.Solid.makeLoft(). The resulting solid IS the inner "
    "void — the exact internal volume that all ribs and spars will be "
    "carved from. "
    "\n"
    "  tip_wire = tip_wire.translate(cq.Vector(0, span, 0))  # span along Y "
    "  core_solid = cq.Solid.makeLoft([root_wire, tip_wire]) "
    "  inner_void = cq.Workplane('XZ').newObject([core_solid]) "
    "\n"
    ...
    "\n\n"
    "STEP 3 — SHELL OUTWARD TO CREATE SKIN: "
    "\n"
    "Take the inner_void solid and shell it OUTWARD using a POSITIVE wall "
    "thickness. This grows the aerodynamic skin around the core. The face "
    "passed to .faces() is the face that is REMOVED (left open). Because "
    "span runs along Y, the root face is ALWAYS '<Y': "
    "\n"
    "  skin = inner_void.faces('<Y').shell(wall_thickness) "
    "\n"
    "Default wall_thickness = 1.5 mm. "
    "\n"
    "WHY '<Y': The root cross-section sits at Y=0 (minimum Y). Selecting "
    "'<Y' removes that face, leaving the root open for structural mounting "
    "to a fuselage or wing box. The full aerodynamic surface (pressure, "
    "suction, leading edge, trailing edge) remains intact. "
    "\n"
    "WHY POSITIVE: A positive value in .shell() grows material OUTWARD "
    "from the existing surfaces. The core solid's surfaces become the "
    "inner mold line, and the new outer surfaces become the aerodynamic "
    "skin. "
    "\n"
    "  # WRONG — selects a Z (thickness) face, rips the aero surface: "
    "  # skin = inner_void.faces('<Z').shell(wall_thickness)  # DESTROYS WING "
    "\n"
    "  # WRONG — selects an X (chord) face, rips the leading/trailing edge: "
    "  # skin = inner_void.faces('<X').shell(wall_thickness)  # DESTROYS WING "
    "\n"
    "STRICT RULE — NEVER CUT THE SKIN: "
    "After shelling, the skin is FINISHED. You MUST NEVER call .cut() on "
    "the skin to carve slots, holes, or cavities. Cutting the skin "
    "destroys the aerodynamic surface continuity. "
    "\n"
    "STRICT RULE — NEVER USE NEGATIVE SHELL + .cut() COMBO: "
    "The old pattern (shell inward, then .cut() to extract the void) is "
    "FATALLY BROKEN due to coincident-face failures in OpenCASCADE. "
    "NEVER write: "
    "  skin = outer_wp.shell(-wall)  # WRONG — shells inward "
    "  inner_void = outer_wp.cut(skin)  # WRONG — coincident face crash "
    "\n\n"
    "STEP 4 — INTERNAL STRUCTURE (Ribs and Spars from inner_void): "
    "\n"
    "Because inner_void is the directly-lofted core solid (not a derived "
    "boolean result), it is guaranteed to be a valid, non-empty body. "
    "Intersecting oversized blanks with it always produces correct geometry. "
    "\n"
    "CHECKPOINT — before writing ANY rib or spar code, verify that: "
    "  (a) inner_void is the lofted core solid (NOT a .cut() result) "
    "  (b) You are NOT calling make_naca_wire() for rib/spar shapes "
    "  (c) Each blank uses .intersect(inner_void) "
    "  (d) Rib blanks are on cq.Workplane('XZ'), extruded/translated along Y "
    "\n"
    "(D1) RIB GENERATION — XZ blank, extrude along Y, intersect inner_void: "
    ...
    "\n"
    "  # WRONG — XY blank with span along Z produces a longitudinal slab: "
    "  # cq.Workplane('XY').rect(...).extrude(t).translate((0, 0, z_pos)) "
    "\n"
    "  # WRONG — clips through shell (matches skin surface): "
    "  # rib = rib_blank.intersect(skin) "
    "\n"
    "  # WRONG — uses make_naca_wire for rib (FORBIDDEN): "
    "  # rib_wire = make_naca_wire(naca, chord) "
    "  # ... any rib from this wire WILL clip through walls "
    "\n"
    "(D2) SPAR GENERATION — YZ blank at chordwise X-offset: "
    ...
    "╚══════════════════════════════════════════════════════════════════════╝\n"
    "\n"
    "The following example demonstrates Core-First Tooling end-to-end "
    "with the XZ-profile / Y-span alignment. Note: wires rotated into XZ, "
    "tip translated along Y, shell on '<Y', ribs on XZ extruded along Y. "
    "No .cut() anywhere. No make_naca_wire() calls after the initial wires. "
    ...
    "  spar_thickness = 2.0 "
    "\n"
    "  # --- Step 1: Generate wires and rotate into XZ plane --- "
    "  root_wire = make_naca_wire(naca, chord_length=chord) "
    "  root_wire = root_wire.rotate((0,0,0), (1,0,0), -90) "
    "  tip_wire = make_naca_wire(naca, chord_length=tip_chord) "
    "  tip_wire = tip_wire.rotate((0,0,0), (1,0,0), -90) "
    "  tip_wire = tip_wire.translate(cq.Vector(0, span, 0)) "
    "\n"
    "  # --- Step 2: CORE-FIRST — loft along Y (this IS inner_void) --- "
    "  core_solid = cq.Solid.makeLoft([root_wire, tip_wire]) "
    "  inner_void = cq.Workplane('XZ').newObject([core_solid]) "
    "\n"
    "  # --- Step 3: Shell OUTWARD (POSITIVE), root face '<Y' left open --- "
    "  skin = inner_void.faces('<Y').shell(wall_thickness) "
    "\n"
    "  # --- Step 4a: Ribs — XZ blank, extrude along Y, intersect inner_void --- "
    ...
    "      ribs.append(rib) "
    "\n"
    "  # --- Step 4b: Spars — YZ blank at chordwise X offset --- "
    "  spar_positions = { "
    "      'spar_front': -chord / 2.0 + chord * 0.25, "
    "      'spar_rear':  -chord / 2.0 + chord * 0.60, "
    "  } "
    "  spars = {} "
    ...
    "\n"
    "  # --- Step 5: Package into Assembly (NO .union(), NO dict result) --- "
    "  assembly = cq.Assembly() "
    "  assembly.add(skin, name='wing_skin') "
    "  for i, rib in enumerate(ribs, 1): "
    "      assembly.add(rib, name=f'rib_{i}') "
