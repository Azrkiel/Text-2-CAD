...
STRATEGY_D = (
    ...
    f"{_SHARED_RULES}"
    "\n\n"
    "DOMAIN D STRATEGY — Master Tooling Pattern: "
    "\n"
    "All Domain D scripts follow one top-down workflow called Master Tooling. "
    "You build the outer mold, shell it, compute the inner void, then derive "
    "every internal part from that void. Nothing is sketched from scratch "
    "inside the wing — everything is carved from the master geometry. "
    "\n\n"
    "╔══════════════════════════════════════════════════════════════════════╗\n"
    "║            NON-NEGOTIABLE GEOMETRIC RULES (ZERO EXCEPTIONS)        ║\n"
    "╚══════════════════════════════════════════════════════════════════════╝\n"
    "\n"
    "The following three rules govern ALL Domain D generation. Violating any "
    "one of them produces geometry that clips, merges, or crashes the pipeline. "
    "There are NO edge cases, NO workarounds, and NO exceptions. "
    "\n\n"
    "── RULE 1: THE GLOBAL MASTER VOID ──────────────────────────────────── "
    "\n"
    "When generating a hollow aerodynamic shape with internal structures "
    "(e.g., a wing with ribs), the script MUST compute the skin AND the "
    "inner_void at the VERY TOP of the script, before generating ANY "
    "internal part. This is the Master Tooling foundation — all downstream "
    "geometry depends on these two objects. "
    "\n"
    "MANDATORY TOP-OF-SCRIPT SEQUENCE (execute these lines first, always): "
    ...
    "of the shell walls. Every rib, spar, and internal member is derived "
    "from this single source of truth by boolean intersection. "
    "\n"
    "If the script generates internal structure and inner_void is not "
    "computed before the first rib or spar, the script is WRONG. "
    "\n\n"
    "── RULE 2: THE BOOLEAN RIB STRATEGY (THE RIB BAN) ─────────────────── "
    "\n"
    ...
    "produces the OUTER mold-line profile at the full chord dimension. Any "
    "rib shaped from that wire matches the outer skin surface and physically "
    "clips through the shelled walls. This is the single most common failure "
    "mode in Domain D generation. "
    "\n"
    "ALL internal structures MUST be derived from inner_void using this "
    "three-step pattern: "
    "  1. Create an OVERSIZED rectangular blank that is larger than the "
    "     airfoil cross-section (e.g., chord*1.2 wide, chord tall). "
    "  2. Translate the blank to the correct spanwise station. "
    ...
    "  - .intersect(outer_wp) instead of .intersect(inner_void) "
    "  - Ribs built without an .intersect() call at all "
    "\n\n"
    "── RULE 3: ASSEMBLY EXPORT ─────────────────────────────────────────── "
    "\n"
    ...
    "╚══════════════════════════════════════════════════════════════════════╝\n"
    "\n\n"
    "STEP 1 — AIRFOIL WIRE GENERATION (USE THE LIBRARY): "
    "\n"
    ...
    "make_naca_wire() returns a closed cq.Wire in the XY plane at Z=0, "
    "centered on the chord midpoint. It uses OCCT B-spline interpolation "
    "with the modified trailing-edge coefficient (-0.1036) for a "
    "mathematically closed trailing edge. "
    "Do NOT manually sketch airfoil curves or approximate with ellipses. "
    "Do NOT use make_naca_airfoil() — use make_naca_wire(). "
    "\n\n"
    "STEP 2 — NATIVE LOFTING (BYPASS .workplane()): "
    "\n"
    "You MUST bypass CadQuery's .workplane() stack for airfoil lofting. "
    "The .workplane() coordinate stack hallucinates internal states when "
    "lofting complex cross-sections. Instead, position wires at global Z "
    "stations with .translate() and loft via cq.Solid.makeLoft(): "
    ...
    "\n\n"
    "STEP 3 — MASTER TOOLING: SHELL + INNER VOID (MANDATORY FIRST): "
    "\n"
    "This step MUST execute immediately after lofting, before ANY internal "
    "structure code. The raw loft is a SOLID block — you must hollow it and "
    "capture the inner void that all ribs/spars will be carved from. "
    "\n"
    "  skin = outer_wp.faces('<Y').shell(-wall_thickness) "
    "  inner_void = outer_wp.cut(skin) "
    "\n"
    "Default wall_thickness = 1.5 mm. The '<Y' selector leaves the root "
    "face open for structural mounting. Adjust if span axis differs. "
    "\n"
    "STRICT RULE — NEVER CUT THE SKIN: "
    "After shelling, the skin is FINISHED. You MUST NEVER call .cut() on "
    "the skin to carve slots, holes, or cavities. Cutting the skin destroys "
    "the aerodynamic surface continuity. "
    "\n\n"
    "STEP 4 — INTERNAL STRUCTURE (Ribs and Spars from inner_void): "
    "\n"
    "CHECKPOINT — before writing ANY rib or spar code, verify that: "
    "  (a) inner_void exists (computed from outer_wp.cut(skin)) "
    "  (b) You are NOT calling make_naca_wire() for rib/spar shapes "
    "  (c) Each blank uses .intersect(inner_void), NOT .intersect(outer_wp) "
    "\n"
    "(D1) RIB GENERATION — oversized blank + boolean intersection: "
    "  For each rib at spanwise station z_pos: "
    "  rib_blank = ( "
    "      cq.Workplane('XZ') "
    ...
    "\n"
    "  # WRONG — clips through shell (outer profile): "
    "  # rib = rib_blank.intersect(outer_wp) "
    "\n"
    ...
    "  and 60% chord from leading edge). Create an oversized rectangular "
    "  blank oriented in YZ, intersect with inner_void: "
    "  spar_blank = ( "
    "      cq.Workplane('YZ') "
    ...
    "  result = assembly "
    "\n"
    "Anchor semantics for the Assembler: "
    ...
    "╚══════════════════════════════════════════════════════════════════════╝\n"
    "\n"
    "The following example demonstrates the Master Tooling pattern end-to-end. "
    "Note the strict order: loft -> shell -> inner_void -> ribs -> spars -> "
    "assembly. No make_naca_wire() calls after the initial loft. No .union(). "
    "No dict assigned to result. "
    "\n\n"
    "  from cad_utils import make_naca_wire "
    ...
    "  spar_thickness = 2.0 "
    "\n"
    "  # --- Step 1: Generate airfoil wires (ONLY use of make_naca_wire) --- "
    "  root_wire = make_naca_wire(naca, chord_length=chord) "
    "  tip_wire = make_naca_wire(naca, chord_length=tip_chord) "
    "  tip_wire = tip_wire.translate(cq.Vector(0, 0, span)) "
    "\n"
    "  # --- Step 2: Loft the outer mold solid --- "
    "  outer_solid = cq.Solid.makeLoft([root_wire, tip_wire]) "
    "  outer_wp = cq.Workplane('XY').newObject([outer_solid]) "
    "\n"
    "  # --- Step 3: MASTER TOOLING — shell + inner void (BEFORE any ribs) --- "
    "  skin = outer_wp.faces('<Y').shell(-wall_thickness) "
    "  inner_void = outer_wp.cut(skin) "
    "\n"
    "  # --- Step 4a: Generate ribs (oversized blank -> intersect inner_void) --- "
    ...
    "      ribs.append(rib) "
    "\n"
    "  # --- Step 4b: Generate spars (oversized blank -> intersect inner_void) --- "
    "  spar_positions = { "
    ...
    "structure), the ONLY safe path is Wire -> Face -> Workplane.add().extrude(): "
